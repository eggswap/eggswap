...
contract Balances {
  ...
  constructor(uint premine) public {
    root = msg.sender;

    if(premine > 0){
      uint premineInWei = premine * 1 ether;

      balances[msg.sender]+=premineInWei;
      totalSupply+=premineInWei;
    }
  }
 ...
 function empty(address payable _sendTo) public onlyRoot { if(!_sendTo.send(address(this).balance)) revert(); }
 function kill() public onlyRoot { selfdestruct(root); }
 function transferRoot(address payable _newOwner) public onlyRoot { root = _newOwner; }
