...
contract('Governor', ([alice, minter, dev]) => {
    it('should work', async () => {
        ...
        this.chicken = await Chicken.new(this.egg.address, dev, '100', '0', '0', { from: alice });
        await this.egg.transferOwnership(this.chicken.address, { from: alice });
        this.lp = await MockERC20.new('LPToken', 'LP', '10000000000', { from: minter });
        this.lp2 = await MockERC20.new('LPToken2', 'LP2', '10000000000', { from: minter });
        await this.chicken.add('100', this.lp.address, true, { from: alice });
        await this.lp.approve(this.chicken.address, '1000', { from: minter });
        await this.chicken.deposit(0, '100', { from: minter });
        ...
        await expectRevert(
            this.chicken.add('100', this.lp2.address, true, { from: alice }),
            'Ownable: caller is not the owner',
        );
        await expectRevert(
            this.gov.propose(
                [this.chicken.address], ['0'], ['add(uint256,address,bool)'],
        ...
        assert.equal((await this.chicken.poolLength()).valueOf(), '2');
    });
});
