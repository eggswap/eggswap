...
contract('Timelock', ([alice, bob, carol, dev, minter]) => {
    ...
    it('should also work with Chicken', async () => {
        ...
        await this.chicken.add('100', this.lp1.address, true);
        await this.chicken.transferOwnership(this.timelock.address, { from: alice });
        const eta = (await time.latest()).add(time.duration.days(4));
        await this.timelock.queueTransaction(
            this.chicken.address, '0', 'set(uint256,uint256,bool)',
        ...
        assert.equal((await this.chicken.poolInfo('0')).valueOf().allocPoint, '200');
        assert.equal((await this.chicken.totalAllocPoint()).valueOf(), '300');
