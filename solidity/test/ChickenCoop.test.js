...
contract('EggToken', ([alice, bob, carol]) => {
    beforeEach(async () => {
        ...
        this.bar = await ChickenCoop.new(this.egg.address, { from: alice });
        this.egg.mint(alice, '100', { from: alice });
        this.egg.mint(bob, '100', { from: alice });
        this.egg.mint(carol, '100', { from: alice });
    });
    ...
    it('should not allow enter if not enough approve', async () => {
        ...
        await this.egg.approve(this.bar.address, '50', { from: alice });
        await expectRevert(
            this.bar.enter('100', { from: alice }),
            'ERC20: transfer amount exceeds allowance',
        );
        await this.egg.approve(this.bar.address, '100', { from: alice });
    ...
    it('should work with more than one participant', async () => {
        await this.egg.approve(this.bar.address, '100', { from: alice });
        await this.egg.approve(this.bar.address, '100', { from: bob });
        ...
        assert.equal((await this.egg.balanceOf(this.bar.address)).valueOf(), '30');
        // ChickenCoop get 20 more EGGs from an external source.
        await this.egg.transfer(this.bar.address, '20', { from: carol });
        // Alice deposits 10 more EGGs. She should receive 10*30/50 = 6 shares.
        ...
        assert.equal((await this.egg.balanceOf(this.bar.address)).valueOf(), '52');
        assert.equal((await this.egg.balanceOf(alice)).valueOf(), '70');
        assert.equal((await this.egg.balanceOf(bob)).valueOf(), '98');
    });
});
